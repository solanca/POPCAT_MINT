...
export default function Home() {
  ...
  useEffect(() => {
    const checkEligibilityFunc = async () => {
      ...
      console.log('guards==',guardReturn);
  ...
  const handleProgress=(minted:number,total:number) => {
    return Math.round((minted/total)*10000)/100
  }
  ...
  const PageContent = () => {
   ...
   `}
            ...
            {loading ? (<></>) : (
                // <Flex justifyContent="flex-end" marginLeft="auto">
                  // <Box background={"teal.100"} borderRadius={"5px"} minWidth={"50px"} minHeight={"50px"} p={2} >
                    <Stack  mt={4}>
                      <Progress value={handleProgress( Number(candyMachine?.itemsRedeemed),Number(candyMachine?.data.itemsAvailable))}/>
                      <Flex justifyContent={"space-between"}>

                      <Text fontWeight={"semibold"}>Total Minted</Text>
                      <Text fontWeight={"semibold"}>{handleProgress( Number(candyMachine?.itemsRedeemed),Number(candyMachine?.data.itemsAvailable))}%({Number(candyMachine?.data.itemsAvailable) - Number(candyMachine?.itemsRedeemed)}/{Number(candyMachine?.data.itemsAvailable)})</Text>
                      </Flex>
        ...
        <Modal isOpen={isShowNftOpen} onClose={onShowNftClose}>
          <ModalOverlay />
          <ModalContent sx={{bgColor:"#FFFAE5"}}>
